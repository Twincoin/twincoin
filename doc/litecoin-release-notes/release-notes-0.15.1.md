Twincoin Core version *0.15.1* is now available from:

  <https://download.twincoin.org/twincoin-0.15.1/>
...
A number of changes to the way Twincoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 lites per kB (0.00001 TWC). This is to prepare for dropping the minimum transaction fee to 0.1 lites per kB (0.0001 TWC) in the next release.
